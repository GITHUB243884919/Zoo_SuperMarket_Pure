...
namespace Game
{
    public partial class LittleZooModule : GameModule
    {
        ...
        public static void LoadExitGate(int sceneID, int idx, float offset)
        {
            int endBuildID = GlobalDataManager.GetInstance().logicTableGroup.GetSingleBuildingID(sceneID, BuildingType.LastBuilding);
            int endResID = Config.buildupConfig.getInstace().getCell(endBuildID).loadresource[0];
...
//        }
        ...
        /// <param name="littleZooID"></param>
        /// <returns></returns>
        public static int MapToFirstSceneLittleZooID(int littleZooID)
        {

            return littleZooID % 1000 + 1000;
        }

        /// <summary>
        /// 查询动物栏是否需要显示动物在场景内
        /// </summary>
        public static bool GetExamineAnimalShowToLittleZooScene(int animalID)
        {
            //利用animalID去查询是否有小类型等级
            //根据等级去返回对应的Bool
            PlayerAnimal_MSS_15 animalMSS15 = GlobalDataManager.GetInstance().playerData.playerZoo.animalMSS15;

            var cellAnimalUp = Config.animalupConfig.getInstace().getCell(animalID);

            bool isShow = animalMSS15.FindAnimalProp(cellAnimalUp.bigtype,cellAnimalUp.smalltype,out AnimalProp animalProp);
            if (isShow&&!animalMSS15.animalIDs.Contains(animalID))
            {
                animalMSS15.animalIDs.Add(animalID);
            }
