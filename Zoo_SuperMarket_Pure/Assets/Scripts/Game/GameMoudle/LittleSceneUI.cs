...
namespace Game
{
    ...
    public class LittleSceneUI 
    {
        ...
        public void Release()
        {
            littleZooMountPoint = null;

            foreach(var val in zooSceneUIDic.Values)
            {
                foreach(var sceneUIArray in val.Values)
                {
                    sceneUIArray.Release();
                }
                val.Clear();
            }
            zooSceneUIDic.Clear();

            ticketMountPoint = null;
            foreach(var val in entrySceneUIDic.Values)
            {
                val.Release();
            }
            entrySceneUIDic.Clear();
