...
namespace CrossRoadGame
{
    ...
    public class StateLoadSceneObject : FSMState
    {
        ...
        protected void LoadEndPos()
        {
            ...
            CrossRoadModelManager.GetInstance().endPos = lastRoadPos
                + Vector3.forward * (endPos.GetComponent<BoxCollider>().size.z * 0.5f + preHalfWidth);
            endPos.transform.position = CrossRoadModelManager.GetInstance().endPos;
        ...
        void GetEndEffect()
        {
            var go = endPos.transform.Find("EndEffectNodeName").gameObject;
            CrossRoadModelManager.GetInstance().endPosEffectSp = new SimpleParticle();
            CrossRoadModelManager.GetInstance().endPosEffectSp.Init(go);
        }
    }
}
