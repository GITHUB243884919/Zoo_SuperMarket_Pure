...
inline float ShoreBlend(float4 screenPos
    ...
    // float diff = sceneZ - screenPos.w;
    // float diff = (1.0-unity_OrthoParams.w*2.0) * (sceneZ - screenPos.z);
    // diff = lerp(10000.0, diff, diff > 0);

    // float depthDifference = sceneZ - screenPos.w;
    float sceneZ = SAMPLE_DEPTH_TEXTURE_PROJ(DEPTHTEX, UNITY_PROJ_COORD(screenPos));
    sceneZ = LinearEyeDepth(sceneZ);;
    float diff = sceneZ - screenPos.z;

    const float segment = 3;
    const float alphaLut[segment] = {0.2, 0.5, 0.7};
    const float depth_size = 10;

    // #ifdef _LIGHTABS_ON
        float seg = depth_size / segment;
        float s = diff / seg;
        float index;
        index = trunc(s);
        abso = lerp(1, alphaLut[index], diff < depth_size);

        // abso = lerp(0.2, diff, step(3, diff));
        // abso = lerp(0.2, lerp(0.5, abso, step(6, abso)), abso != 0.2);
        // abso = lerp(0.2, lerp(0.5, lerp(0.7, abso, step(9, abso)), abso != 0.5), abso != 0.2);
        // abso = saturate(abso);
        // abso = saturate(diff);
    // #endif
