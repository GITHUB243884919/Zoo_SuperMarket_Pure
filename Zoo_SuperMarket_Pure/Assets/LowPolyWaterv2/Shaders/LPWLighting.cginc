...
inline fixed4 LPWLightingFrag(LPWLightingInput i){
    ...
    half3 watCol = half3(1, 1, 1);
    half opacity = _Opacity;

    // #if defined(_LIGHTABS_ON)
        half4 deepCol = _DeepColor;
        // half3 watColOpacity = lerp(deepCol, _Color.rgb, i.abso);
        // half3 watColOpacity = lerp(_Color.rgb, deepCol, i.abso < 1);
        // watColOpacity = lerp(half3(0,1,0), watColOpacity, i.abso);
        // watColOpacity = lerp(half3(0,0,1), watColOpacity, i.abso);
        watCol = _Color.rgb;
